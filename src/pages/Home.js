...
const Home = () => {
  ...
  const [isFolder, setIsFolder] = useState(false); // track whether file of folder is being created
  const [fileName, setFileName] = useState("");
  const [renameDeleteTracker, setRenameDeleteTracker] = useState({});
  const [isDuplicate, setIsDuplicate] = useState(false);
  const [activeFolder, setActiveFolder] = useState({});
  const [isBackDisabled, setIsBackDisabled] = useState(true);
  const [action, setAction] = useState("into"); // to track whether traversing "into" or "back"
  ...
  const [activeFolderContents, setActiveFolderContents] = useState(
    fileFolder.rootTree
  );
